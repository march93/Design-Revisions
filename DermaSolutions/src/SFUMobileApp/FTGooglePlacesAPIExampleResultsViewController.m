...
- (id)initWithGooglePlacesAPIRequest:(id<FTGooglePlacesAPIRequest>)request locationCoordinate:(CLLocationCoordinate2D)locationCoordinate;
{
    ...
    //NSLog(@"Sent Requests");
    //NSLog(@"Location Coordinate = %@", locationCoordinate);
...
- (void)viewDidLoad
{
    ...
    id<FTGooglePlacesAPIRequest> request = [self googlePlacesAPIRequestForExampleType:1];
   [self initWithGooglePlacesAPIRequest:request locationCoordinate:self.locationCoordinate];
   
    //As required by Google, we must put a "Powered by Google" logo in the bottom left of the view.
    UIImageView *dot =[[UIImageView alloc] initWithFrame:CGRectMake(10,640,104,16)];
    dot.image=[UIImage imageNamed:@"powered-by-google-on-white@2x.png"];
    //NSLog(@"\n\n\nThis is the image %@\n\n\n",dot.image);
